...
class category extends My_Controller {
    ...
    */
    ...
    public function add() {
        $this->s_title .= " :: Category Addition";
      ...
      //  $this->s_sub_menu_id = 'add_user';
        ...
        if(count($_POST)>0)
        {
            ...
            {
                $m_send_data['cat_name'] = get_safe($this->input->post("cat_name"));
                  // Inserting user data into database
                $i_inserted_id = $this->mod->insertData('category', $m_send_data);
                if($i_inserted_id>0) {
                    // Adding message
                    add_msg("Category added successfully.", "ok");
                    // redirecting the user to home page
                    redirect(admin_url().'category/listing.html');
                } else {
    ...
    */
    public function listing($i_page=''){
        $this->s_title .= " :: Category Listing";

        $this->load->library('pagination');

        $this->s_sub_menu_id = 'list_category';
    ...
    */
    public function edit($s_edit_id='') {
        $this->s_title .= " :: Category Edit";
        // setting sub-menu items
        $this->s_sub_menu_id = 'add_user';
        $m_send_data = array();

        $i_edit_id = intval(strDecode($s_edit_id));
        if($i_edit_id<1)redirect(base_url().'category/listing.html');
        ...
        if(count($retset)==0){
            add_msg("No Category details found to edit", "info");
            redirect(admin_url().'category/listing.html');
        }
    ...
    */
    public function del($s_id='') {
        ...
        $ret_ = $this->mod->delData('category', $m_where);
        // Message setting
        if($ret_>0){
            add_msg('Category deleted successfully!!', "ok");
        }else{
            add_msg('Category not deleted!!', "err");
        }
        redirect(admin_url().'category/listing.html');
    }
