...
class Affiliate extends My_Controller {
    ...
    */
    public function add(){
        $this->session->unset_userdata('date_val');
        $this->s_title .= " :: Affiliation Addition";
        $this->s_sub_menu_id = 'add_affle';
        // $this->chk_user_access('admin');

        $sess_data=$this->session->userdata('date_val');
        if(!empty($sess_data)){
            $this->session->unset_userdata('date_val');
        } 
        ...
        if(count($_POST)>0) {
            try {
                ...
                $this->form_validation->set_rules('afname', 'Affiliate Name', 'required|is_unique[affiliation_details.s_aff_name]|callback_alpha_dash_space');
                $this->form_validation->set_rules('afemail', 'Affiliate Email', 'required|is_unique[affiliation_details.s_aff_email]');
                $this->form_validation->set_rules('doa', 'Affiliate Date', 'required');
                $this->form_validation->set_rules('afurl', 'Product Affiliate Code', 'required|is_unique[affiliation_details.s_aff_url]');
                $this->form_validation->set_rules('asfurl', 'SEO Affiliate URL', 'required|is_unique[affiliation_details.s_seo_aff_url]');
    ...
    */
    ...
    public function listing($i_page=""){
        ...
        if ($this->form_validation->run() !== FALSE)
        {
            ...
            $this->session->set_userdata('date_val',$date_val);
            ...
            $sess_date=$this->session->userdata('date_val');
            if(!empty($sess_date)){
                //   pr($sess_date,true);
                $s_tab_name='affiliation_details';
                $s_select='*';
                $m_where="`t_aff_time` between '".$sess_date[0]."' and '".$sess_date[1]."'";
                $s_group_by = 'i_aff_id';
                $s_order_by_name = "i_aff_id ";
                $s_order_by = "ASC";
                $i_perpage=$this->per_page_show;
        ...
        $this->m_data['date_val']=$date_val;
        //  $this->session->userdata('date_val');
        $this->render();
    }
    ...
    */
    public function edit($s_edit_id='') {
        $this->session->set_userdata('date_val');
        ...
        if(count($_POST)>0) {
            ...
            try {
                ...
                $this->form_validation->set_rules('afemail', 'Affiliate Email', 'required');  
                $this->form_validation->set_rules('doa', 'Affiliate Date', 'required');
                $this->form_validation->set_rules('afurl', 'Product Affiliate Code', 'required|callback_affiliate_code_check');
                $this->form_validation->set_rules('asfurl','SEO Affiliate Code','required');
                $this->form_validation->set_rules('aserfurl','Service Affiliate Code','required');
                /// Validation rule setting [end]

                $m_send_data[0]['s_aff_name'] = get_safe($this->input->post("afname"));
                $m_send_data[0]['s_aff_email'] = get_safe($this->input->post("afemail"));
                $adate=date_format(date_create($this->input->post('doa')),'Y-m-d');
                $m_send_data[0]['t_aff_time'] = $adate;
                $m_send_data[0]['s_aff_url'] = $this->input->post("afurl");
                $m_send_data[0]['s_seo_aff_url'] = $this->input->post("asfurl");
                $m_send_data[0]['s_service_aff_url'] = $this->input->post("aserfurl");
                $m_send_data[0]['i_aff_id'] = $this->input->post("val1");  
    ...
    public function search($i_page=""){
        ...
        if($this->form_validation->run()!=FALSE){
            ...
            $s_tab_name='affiliation_details';
            $s_select='*';
            $m_where=array('i_aff_id'=>$s_id,'s_aff_name'=>$s_id,'s_aff_email'=>$s_id,'t_aff_time'=>$s_id,'s_aff_url'=>$s_id); 
            // if($i_user_role>0)
            // $m_where=array();
            $s_group_by='';
            $s_order_by_name = 'i_aff_id';
            ...
            $config['base_url']=admin_url()."affiliate/search/"; 
            ...
            if(!empty($sess_id)){
                $s_tab_name='affiliation_details';
                $s_select='*';
                $m_where=array('i_aff_id'=>$sess_id[0],'s_aff_name'=>$sess_id[0],'t_aff_time'=>$sess_id[0],'s_aff_url'=>$sess_id[0]); 
                // if($i_user_role>0)
                // $m_where=array();
                $s_group_by='';
                $s_order_by_name = 'i_aff_id';
    ...
    public function affiliatetrack($i_page=''){
        $this->session->unset_userdata('arr_elm_track');
    ...
    public function affilatetrackdetprop(){
        //echo $s_aff_id;exit; 
        $this->session->unset_userdata('arr_elm_track1'); 
        //    redirect(admin_url()."affiliate/afftrackdetails/".$s_aff_id);
    ...
    public function afftrackdetails($s_aff_id="",$i_page=""){
       ...
       $this->session->unset_userdata('arr_elm_track');
        ...
        if($this->form_validation->run()!=FALSE){
            ...
            $arr_elm=array($s_id);
            $this->session->set_userdata('arr_elm_track1',$arr_elm);

            $s_tab_name='influxiq_affliationhit_details';
            //$s_select='*';
            $s_select='*'; 
            //$m_where=array('i_affhit_id'=>$s_id,'t_affhit_time'=>$s_id,'s_affhit_ip'=>$s_id); 
            // $m_where1=array('i_aff_id');  
            $m_where=array($aff_id,$s_id);
            ...
            $retset1=$this->mod_af->fetchSearchUser1($s_tab_name, $s_select, $m_where,$s_group_by,$s_order_by_name); 
            $retset=$this->mod_af->fetchSearchUser1($s_tab_name, $s_select, $m_where, $s_group_by, $s_order_by_name,$s_order_by,$this->per_page_show,$i_page);   
            $this->m_data['m_dataset']= $retset;
            //  pr($retset1,true);
            //$this->m_data['search_arr']=$retset;
            ...
            // $this->m_data['search_arr']=$this->mod_af->fetchSearchAfftrackDet("credit_affliationhit_details","*",$s_id,$i_aff_id);
            /*if(empty($this->m_data['search_arr'])){
            redirect(admin_url()."affiliate/affsearchtrackdet/".$s_aff_id); 
            }   */
        }else{
            $arr=$this->session->userdata('arr_elm_track1');
            if(!empty($arr)){
                $s_tab_name='credit_affliationhit_details';
                //$s_select='*';
                $s_select='*'; 
                //$m_where=array('i_affhit_id'=>$arr[0],'t_affhit_time'=>$arr[0],'s_affhit_ip'=>$arr[0]); 
                // $m_where1=array('i_aff_id');  
                $m_where=array($aff_id,$arr[0]);
            ...
            $retset1=$this->product_model->fetchMultiData($s_tab_name, $s_select, $m_where, $s_group_by, $s_order_by_name, $s_order_by);
            $retset=$this->product_model->fetchMultiData($s_tab_name, $s_select, $m_where, $s_group_by, $s_order_by_name, $s_order_by,$i_perpage,$i_page);    
            $this->m_data['m_dataset']= $retset;
            $config['base_url']=admin_url()."affiliate/afftrackdetails/".$s_aff_id."/"; 
            $config['total_rows']=count($retset1);
            $config['per_page']=$this->per_page_show;
   ...
   //     if($flag==0)
    ...
    public function affsearchtrack($i_page=""){
        ...
        if($this->form_validation->run()!=FALSE){
            ...
            $this->m_data['search_arr']=$retset;
            $config['base_url']=admin_url()."affiliate/affsearchtrack/"; 
            $config['total_rows']=count($retset1);
            $config['per_page']=$this->per_page_show;
            $config['num_links'] = 4;
            $config['uri_segment'] = 4;
            ...
            $sess_id=$this->session->userdata('arr_elm_track');
            //  pr($sess_id,true);
            if(!empty($sess_id)){
                $s_tab_name='vw_affiliatehit_details';
                //$s_select='*';
                $s_select='i_aff_id,s_aff_name,s_affhit_ip,t_affhit_time,count(`i_aff_id`) as `counthit`'; 
                $m_where=array('i_aff_id'=>$sess_id[0],'s_aff_name'=>$sess_id[0]); 
                // if($i_user_role>0)
                // $m_where=array();
                $s_group_by='i_aff_id';
                $s_order_by_name = 'i_aff_id';
    ...
    public function affsearchtrackdet($s_aff_id='',$i_page=""){
        ...
        if($this->form_validation->run()!=FALSE){
            ...
            if(!empty($sess_id)){
                ...
                //$s_select='*';
                $s_select='*'; 
                $m_where=array('i_affhit_id'=>$sess_id[0],'t_affhit_time'=>$sess_id[0],'s_affhit_ip'=>$sess_id[0]); 
                $m_where1=array('i_aff_id');  
    ...
    function download(){
        ...
        if(empty($arr1)){
            $s_tab_name='influxiq_affiliation_details';
    ...
    function download1(){
        ...
        $s_select="i_aff_id,s_aff_name,t_aff_time,s_aff_url";
        $retset=$this->mod_af->fetchSearchUser("influxiq_affiliation_details",$s_select,$val[0]);
        //pr($retset,true);
    ...
    function downloadafftrackdetails($s_aff_id=""){
        $this->load->helper('csv');
        //  $var_arr=$this->session->set_userdata("retset");
        $var1=array();
        $var1=$this->session->userdata('arr_elm_track1');
        $s_tab_name='influxiq_affliationhit_details';
        //$s_select='*';
        $s_select="i_affhit_id,t_affhit_time,s_aff_type,s_affhit_ip"; 
        if(!empty($var1)){

            //$m_where=array('i_affhit_id'=>$s_id,'t_affhit_time'=>$s_id,'s_affhit_ip'=>$s_id); 
            // $m_where1=array('i_aff_id');  
            $m_where=array($s_aff_id,$var1[0]);
            ...
            $retset=$this->mod_af->fetchSearchUser1($s_tab_name, $s_select, $m_where,$s_group_by,$s_order_by_name); 
        }
        else{
        ...
        //pr($retset,true);
        $heading=array('Affiliate ID','Hitting Time','Hitting Affiliate Type','Hitting IP');
        $result1[0]=$heading;
    ...
    function downloadaffserchtrackdetails($s_aff_id=""){
        ...
        $retset=$this->mod_af->fetchSearchAfftrackDet("credit_affliationhit_details","i_affhit_id,t_affhit_time,s_affhit_ip",$sess_aff[0],$i_aff_id);
    ...
    function downloadaffsearchtrack(){
        ...
        //pr($val,true);
        /* $s_tab_name='credit_vw_affiliatehit_details';
        $s_select='i_aff_id,s_aff_name,count(`i_aff_id`) as `counthit`';
        ...
        */
        ...
        //$s_select='*';
        $s_select='i_aff_id,s_aff_name,count(`i_aff_id`) as `counthit`'; 
        $m_where=array('i_aff_id'=>$val[0],'s_aff_name'=>$val[0]); 
        // if($i_user_role>0)
        // $m_where=array();
        $s_group_by='i_aff_id';
        $s_order_by_name = 'i_aff_id';
        ...
        $retset=$this->mod_af->fetchAffiliate1($s_tab_name, $s_select, $m_where,$s_group_by,$s_order_by_name);
        //pr($retset,true);
    ...
    */
    ...
    public function affiliate_code_check($s_aff_code) {
        ...
        //$arr1=explode('~',$val);
        // pr($arr1,TRUE);
        // $ret1_=$this->mod_af->fetchMultiDataCount('affiliation_details',array('s_aff_url'=>$s_aff_code));
        $o_res=$this->db->query("select * from influxiq_affiliation_details where `i_aff_id`!='".$val."' and `s_aff_url`='".$s_aff_code."'");
