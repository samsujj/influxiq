...
function getProductName($i_prod_id){
    $CI = & get_instance();
    $o_res = $CI->db->select('name')->from('product')->where(array('id'=>$i_prod_id))->get();
    $m_res = $o_res->result_array();
    return($m_res[0]['name']);
}


function getAffiliateName($type,$aff_url){
    if($type=="product"){
        $indx='s_aff_url';
    }
    else if($type=="seo"){
            ...
            $indx='s_service_aff_url'; 
    }
    $CI = & get_instance();
    $o_res = $CI->db->select('s_aff_name')->from('affiliation_details')->where(array($indx=>$aff_url))->get();
    $m_res = $o_res->result_array();
    return($m_res[0]['s_aff_name']);
...
function is_superadmin(){
    $CI = & get_instance();
    $m_login_info = $CI->session->userdata('ses_user_data');
    //pr($m_login_info);exit;
    if(!empty($m_login_info) && $m_login_info['i_user_id']>0){
        return (strtolower($m_login_info['i_user_role'])==1)?TRUE:FALSE;
    }
    return FALSE;    
}
...
function getStateName($i_state_id=""){
    $CI = & get_instance();
    if(!empty($i_state_id)){
        $sql="select st_name from influxiq_state where id=".$i_state_id."";
    }
    else{
           $sql="select st_name from influxiq_state";
    }
    //  $o_res = $CI->db->select('i_id,s_aff_type')->from('influxiq_affiliation_type')->get();
    $o_res=$CI->db->query($sql);
    $m_res=$o_res->row_array();
    return($m_res['st_name']);
}
...
function is_admin(){
    ...
    $m_login_info = $CI->session->userdata('ses_user_data');
    if(!empty($m_login_info) && $m_login_info['i_user_id']>0){
        return (strtolower($m_login_info['i_user_role'])==2)?TRUE:FALSE;
    }
    return FALSE;    
}
...
function is_plat_cart($cart=array()){
    if(count($cart)){
        foreach($cart as $row){
            if($row['type'] ==1){
                return true;
...
function get_plat_id($cart=array()){
