...
jQuery(function()
{
   ...
   jQuery(".removeBut").click(function(){
	...
	jQuery(".addnty").click(function(){
        ...
        jQuery.post('<?php echo base_url() ?>contract/updateCart1',{'prod_id':prod_id,'land_numb':land_numb},
			function(res){				
				if(res==1){
	...
	jQuery(".unsetCart").click(function(){
        jQuery.get('<?php echo base_url(); ?>home/unsetCart',{},function(html){
			if(html==""){
                  ...
                  window.location.href='<?php echo base_url(); ?>home/products.html';
			}
			});
      //       
	});
   ...
   jQuery("#submitBtn").click(function(){
       ...
       $('#dig_sig').removeAttr('style');
       $('#dig_text').removeAttr('style');
    ...
    function email_contract(){
        $('#dig_sig').css('display','none');
        $('#dig_text').css('display','none');
        jQuery("#contact_form").attr('action','<?php echo base_url()?>contract/add_contract');
...
<div style="width:960px; margin:0 auto;">
  ...
  <div style="width:960px; margin:0 0 10px 0;">
	...
	<?php if(!empty($row[0]['image'])){ ?>	
      ...
      <img src="<?=base_url()?>images/partitionc.jpg" />
       ...
       ?>
           <?php
              ...
              {
           ?>
          Platform Cost : <strong style="font-weight:normal; color:#e07300; margin-top:10px;">$<?php echo $row[0]['price'];?></strong> <br>
       ...
       Quantity :&nbsp;
           <select id="qnty" style="width: 60px; border: solid 1px #ddd;">
               <?php for($i=1;$i<=10;$i++){ ?>
                   <option value="<?php echo $i;?>" <?php if($row['qty']==$i) {?> selected="selected" <?php } ?>><?php echo $i;?></option>
               <?php } ?>
           </select>
         </div>
      ...
      <br/><input type="button" style="background:url(http://influxiq.com/images/remove.jpg) no-repeat; width:138px; height:41px; cursor:pointer; border:none; margin:10px 0;" value="" class="removeBut" attr="<?php echo $row[0]['id']; ?>">
 ...
 <div style="width:960px; height:90px; display:table-cell; vertical-align:middle;">
  ...
  <?php echo put_safe(@$m_dataset[0]['terms_conditions']);?> 
      ...
      <table width="45%" border="0" cellspacing="0" cellpadding="0" style="float:left; margin:10px 0 0 15px;">
          ...
          <tr id="dig_text" style="display: none;">
            ...
            <tr id="dig_sig" style="display: none;">
            ...
            <tr>
              <td height="50">&nbsp;</td>
              <td height="50">
                  <input type="hidden" name="det_val" value='<?php echo serialize(@$m_cart_val);?>' />
                  <input id="submitBtn" type="submit" value="" style="background:url(<?php echo base_url();?>images/submit2.jpg) no-repeat; background-size: 125px; width:125px; height:41px; cursor:pointer; border:none; margin:10px 0;" />
                  <input type="button" value="" style="background:url(<?php echo base_url();?>images/submit3.jpg) no-repeat;  background-size: 125px; width:125px; height:41px; cursor:pointer; border:none; margin:10px 0;" onclick="email_contract()"/>
                <p  style="font-family:Arial, Helvetica, sans-serif; font-size:12px; color:#ff0000; margin:0; padding:0;"></p></td>
            </tr>
          </table>
          
          </form>
           <form action="<?php echo base_url();?>contract/add_contract" method="post">
          ...
          <br/><input type="button" style="background:url(http://influxiq.com/images/startover.jpg) no-repeat; width:138px; height:41px; cursor:pointer; border:none; margin:10px 0;" value="" class="unsetCart">
          <?php } ?>
          </span> </div>
