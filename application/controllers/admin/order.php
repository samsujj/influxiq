...
class Order extends My_Controller {
    ...
    private $i_edit_id = 0;
    public $per_page_show = 5;  

    public function __construct(){
        parent::__construct();
        $this->chk_user_access('admin');
        $this->load->library('form_validation');
        $this->load->model('my_model','mod_af');
        $this->load->helper('dropdown_helper');
        $this->load->helper('image');
        $this->s_menu_id = 'menu_order';
    }

    public function index() {
        $this->s_menu_id = 'menu_order';
        redirect(admin_url()."order/listing");
    }
    ...
    */
    public function listing($i_page=""){
        $this->s_title .= " :: Order Listing";
        $this->s_sub_menu_id = 'list_order';

        $this->load->library('pagination');  
        $flag=0;
        //  $sdate=$this->input->post('sdate');
        //  $fdate=$this->input->post('fdate');
        $s_search_val=$this->session->userdata('s_search_val'); 
        ...
        if($flag==0){
            ...
            $s_select='*';
            if(!empty($s_search_val)){
                $m_where=array('aff_name'=>$s_search_val);
            }
            else{
                $m_where=array(); 
            }
    ...
    */
    ...
    public function search($s_action=""){
        if(count($_POST)>0){
            $s_search=$this->input->post('aff_name');
            $this->session->set_userdata('s_search_val',$s_search);
        }
        if($s_action=="all"){
            $this->session->unset_userdata('s_search_val');   
        }
