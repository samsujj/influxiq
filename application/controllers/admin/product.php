...
class product extends My_Controller {
    ...
    */
    ...
    public function change_state($s_state='', $s_enc_id='',$s_back_url=""){
        ...
        $s_back_url = (!empty($s_back_url))?base_url().strDecode($s_back_url):admin_url()."user/listing.html";
       

        $ret_ = $this->mod->updateData('product', array('i_active'=>$i_is_allow), array('id'=>$i_id));
        if($ret_){
            ($i_is_allow==1)?add_msg("Product is active now.", 'ok'):add_msg("Product is inactive now", 'ok');
        }else{
